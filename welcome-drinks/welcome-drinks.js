...
async function loadData(){
  let res=await api("/welcome-drinks");
  if(res.status===403){
    await promptForLogin();
    await loadSession();
    res=await api("/welcome-drinks");
  }
  if(!res.ok)throw new Error("failed to load welcome drinks data");
  const body=await res.json();
  data=body.data;
...
async function save(){
  const res=await api("/welcome-drinks",{method:"PUT",body:JSON.stringify({data,revisionId})});
  if(res.ok){
    const body=await res.json();
    revisionId=body.revisionId;
    editing=false;
...
async function openHistory(){
  ...
  const res=await api("/welcome-drinks/revisions");
...
async function compareRevision(id){
  const res=await api(`/welcome-drinks/revisions/${encodeURIComponent(id)}/diff`);
  if(!res.ok){
...
async function loadConfig(){
  ...
  document.title="Welcome Drinks"+(config.venueName?" · "+config.venueName:"");
