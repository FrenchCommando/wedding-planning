...
- Stationery — same architecture as the other sub-projects: own Drive file (`stationery.json`), own router (`src/routes/stationery.ts`), own page (`stationery/`, split `.html`/`.css`/`.js`), History/diff from day one (`src/stationery-diff-config.ts`). One flat `items` list — every printed/written piece (save-the-dates, invitations, menus, place cards, signage, thank-yous) with `status`, `quantity`, `vendor`, `dueDate`, `cost`, `notes`. `status` is a **closed list** (`To do / Designing / Proofing / Ordered / Received / Sent`, in `stationery.js`) rather than free text, so the header summary can count what's done and the status pill can style itself without string-matching prose; a `dueDate` in the past renders red only while the piece isn't Received/Sent. Header line shows piece count / done count / total cost. Nav-visible to `editor`/`vendor` only (organizer + stationer data, not guest-facing). **"Print name list"** is the fourth renderer of seat data (after the live plan, `readonly.css`'s export and the seating chart's own print SVG) — it reads `GET /api/seating` live and prints table-by-table lists for whoever writes the place cards, so it carries names and nothing else: no dietary markers, no pending-RSVP markers, no party colours, no plan, no list numbering. `+N` guest records collapse to one line per household with the **total** headcount in parentheses (`陈林湖 +1`/`+2` → `陈林湖 (Chen Linhu) (3)`), keyed on the base name so a household split across tables would collapse per table. The rule is on the `+N` convention, not on script — the one non-Chinese case, `Emmanuel Ren +1`, collapses the same way. Initial/bulk content goes in via the usual `npx tsx src/seed-sync.ts stationery` path (`data/stationery-seed.json`, gitignored; shape in `data/stationery-seed.json.example`) rather than being left to first-load-creates-an-empty-file. Verified against the dev server with `curl` (GET/PUT/stale-PUT 409/revisions/diff, plus the anon-403 and vendor-read-only-403 gates) and `tsc --noEmit`/`node --check` — not click-tested in a browser.
