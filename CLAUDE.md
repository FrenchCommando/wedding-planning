...
- Stationery — same architecture as the other sub-projects: own Drive file (`stationery.json`), own router (`src/routes/stationery.ts`), own page (`stationery/`), History/diff (`src/stationery-diff-config.ts`). One flat `items` list, one row per printed piece, each carrying `status` (closed list, `To do` renders no pill), `quantity`, `vendor`, `dueDate`, `cost`, `notes` (the brief — spec and what was decided with the designer), `wording` (the copy that gets printed) and `noDownload`. Rows open by default; the copy inside each collapses behind its own summary.
  ...
  - **Table plan card mockup and print sheet.** Recreation's first proof (`H&M Plan de table 12x17.pdf`, 2026-09-09) laid the card out as: number large top-left, salon in italics right-of-centre with a short rule, names right-aligned at the foot in spaced capitals. Its Chinese tables were broken — the layout face had no CJK glyphs so every name printed as `(    ) REN KEWU`, and long pinyin wrapped mid-name. The in-app mockup (`planCard` in `stationery.js`) now follows that layout, and `stationery/plan-cards.html` (+`.css`/`.js`, linked from the stepper bar under the mockup) renders every seated table as a real 12×17cm page from the live chart with `@page` sizing and a CJK serif (Noto Serif SC via Google Fonts, so it needs network), for printing/saving as PDF and sending back as the reference. Entries are set like film credits (`splitEntry`): a two-column grid on one central gutter, packed to the right — characters + 两位 count right-aligned into the gutter, romanisation in spaced capitals left-aligned out of it (Martial tried both-columns-right-aligned spread across the box on 2026-09-09 and chose this one); a Latin name spans both columns and right-aligns, so an all-Latin table reads exactly as the proof's. Nothing wraps. The dev seeds (`seating-chart/seating-seed-dev.json`, `stationery/stationery-seed-dev.json`) carry two salons, a Chinese-household table with +1/+2 records and a long pinyin name, and the plan-cards/table-numbers pieces, so this can be demoed on dev. Names auto-shrink per card until they fit (`fitPlanCards` / `fit`), with an A−/A+/Auto stepper on both pages pinning one size across all cards — a per-browser localStorage preference, not stationery data. Name handling shared by both pages lives in `stationery/names.js`. Confirmed working in a browser by Martial on 2026-09-11.
