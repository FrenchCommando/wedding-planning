...
- Transportation — same architecture as welcome-drinks: own Drive file (`transportation.json`), own router (`src/routes/transportation.ts`), own page (`transportation/`), History/diff (`src/transportation-diff-config.ts`). Two collections: `shuttles` (one row per departure: `time`, `direction` = `toVenue`/`back`, `from`, `to`, `capacity`, `notes`) and `riders` (`name`, `household`, `toVenue`/`back` booleans, `notes`), plus the same root `extraCount`/`extraCountNote` pair as the events pages (assumed to ride both ways). The shuttle list is grouped by leg with a "N seats for M" line per leg, red when short. **Guest-visible, unlike the events pages** — the timetable is the guest's own ride — but the GET strips `riders`/`extraCount` for the guest role server-side, and `mountRevisionRoutes` grew an optional `roles` so History (whose diffs name riders) is editor/vendor only here; the page hides the rider section and the History button when the response has no `riders`. The rider list was derived from `guests-import.json` (RSVP `Confirmed` and yes to at least one shuttle question — Yes/Oui/是的) and **pushed to prod on 2026-09-13 through `PUT /api/transportation`** on a spare-port instance, not via a seed: 76 riders, 75 to the venue, 76 back, no shuttles yet since none had been scheduled. The interim `data/transportation-seed.json` was deleted the same day. Verified via the dev server + `curl`/`tsc --noEmit`/`node --check` (anon-403, vendor-PUT-403, PUT, stale-PUT 409, revisions/diff, guest GET stripped, guest revisions 403, page 200), not click-tested in a browser.
